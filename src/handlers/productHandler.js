...
export const getProductByIdHandler = async (req, res) => {
  ...
  try {
    const [products] = await pool.query(
      "SELECT user_id, name, description, price, stock, created_at, updated_at FROM products WHERE id=?",
      [id]
    );
...
export const addProductHandler = async (req, res) => {
  ...
  if (name.includes(" ")) {
    return res
      .status(400)
      .json({ status: "fail", message: "Name cannot contain spaces" });
  }
  ...
  try {
    ...
    if (user.length === 0)
      return res
        .status(400)
        .json({ status: "fail", message: "Invalid user_id" });
...
export const deleteProductHandler = async (req, res) => {
  ...
  try {
    const [deleteProduct] = await pool.query("DELETE FROM users WHERE id=?", [
      id,
    ]);
